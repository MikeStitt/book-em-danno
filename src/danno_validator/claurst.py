...
Scope: only **local Ollama** is enabled end-to-end today. The fork build DOES honor
the egress proxy (reached NVIDIA NIM in the spike), so cloud is reachable in principle,
but cloud model SELECTION is still gated loud in `sandbox.resolve_claurst_model`
pending the Part-3 cloud lift — so a cloud ref fails at launch, not mid-session.
"""
