...
def run_validate(
    ...
    *,
    ...
    with run_log(plan.out_dir / "danno.log"):
        ...
        # #101: snapshot each sandbox's egress reachability log (host-side sbx audit) while
        # the VMs are still alive — BEFORE teardown, and regardless of --keep-sandboxes. Only
        # under --capture, alongside the wire JSONL; a no-op on the docker backend.
        if opts.capture:
