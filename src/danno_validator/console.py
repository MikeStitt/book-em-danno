...
unit-testable with a no-op `Reporter`; this rendering is plain I/O and is not
unit-tested.

Channel split (Printing & Error-Reporting policy §4D): **progress** — the plan
preamble, phase beats, per-tier live status, the menu-copy hint, and the strict
failure notice — is operational narration and goes to `err_console` (**stderr**),
so it never pollutes the parseable product. The **results summary** — the grid,
the swept counts/taxonomy/baseline, and the artifact paths — is the command's data
product and stays on `console` (**stdout**).
"""
...
from book_em_danno.core.exec import console, err_console
...
class ConsoleReporter(Reporter):
    """Stream a validate run: progress → `err_console` (stderr), summary → `console`
    (stdout). See the module docstring for the channel split."""
    ...
    def plan(self, plan: ValidatePlan, *, dry_run: bool) -> None:
        ...
        for label, value in rows:
            err_console.print(f"  {label:11s} {value}")
        err_console.print(f"\n  disposable sandbox (sweep):    {plan.sweep_sandbox}")
        if plan.baseline_sandbox is not None:
            err_console.print(f"  disposable sandbox (baseline): {plan.baseline_sandbox}")
        err_console.print(
    ...
    def event(self, ev: ValidateEvent) -> None:
        if ev.phase == "config-start":
            ...
            err_console.print(f"    L{ev.level} {ev.label} … [dim]running[/dim]")
            ...
            err_console.print(f"    L{ev.level}  {label}   [dim]({ev.latency_s:.1f}s{tok})[/dim]")
        elif ev.phase == "tier-skip":
            err_console.print(f"    [dim]→ L{ev.level} {ev.label} skipped ({ev.reason})[/dim]")
