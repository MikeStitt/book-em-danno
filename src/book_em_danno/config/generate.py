...
_HEADER = (
    ...
    "// provider.options.num_ctx  : Ollama's REAL context window (the KV-cache it loads);\n"
    "//                             costs RAM/VRAM. Exceed it and Ollama silently truncates.\n"
    "// models.<tag>.limit.context: OpenCode's BELIEF of the window (used to trim/compact).\n"
    "//                             Kept == num_ctx, else OpenCode overpacks → truncation.\n"
    "// models.<tag>.limit.output : tokens OpenCode reserves for the reply\n"
    "//                             (usable input is roughly context - output).\n"
    "// provider.options.stream   : stream tokens from Ollama (runtime-significant; keep true).\n"
    "// provider.options.thinking : enable the model's 'thinking' channel (runtime-significant).\n"
    ...
    for model_name in sorted(used_models):
        ...
        if isinstance(backend, OllamaBackend):
            ...
            provider["models"][model.tag] = {
                ...
                "limit": {"context": backend.num_ctx, "output": backend.output_limit},
            }
