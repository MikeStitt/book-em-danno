...
from .schema import DannoConfig, LlamacppBackend, OllamaBackend, OpenAIBackend
...
def agent_ref(config: DannoConfig, value: str) -> str:
    """Resolve an [agents] value to an OpenCode model ref.
...
def render_config(config: DannoConfig) -> str:
    ...
    agent_block = {
        agent: {"model": agent_ref(config, value)} for agent, value in config.agents.items()
    }

    default_agent = config.defaults.default_agent
    if default_agent in config.agents:
        main_ref = agent_ref(config, config.agents[default_agent])
    elif config.models:
